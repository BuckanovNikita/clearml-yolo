...
### Патч распознавания трансформов

Ultralytics решает, передавать ли в свой `A.Compose` параметр `bbox_params`, по списку из
39 имён классов. Всё, чего в списке нет — трансформ внутри `A.OneOf`, ваш собственный
подкласс, класс, добавленный в albumentations позже, — считается пиксельным: картинка
поворачивается, а боксы остаются на месте, и разметка молча перестаёт совпадать.

`clearml_yolo.ultralytics_patch` заменяет проверку по имени на тот вопрос, ради которого
список и писался: есть ли в пайплайне `A.DualTransform` на любой глубине вложенности.
Патч применяется автоматически в `cy-train` и `cy` и дописывается в модуль, который
ultralytics генерирует для DDP-процессов, — иначе каждый ранг импортировал бы чистый
ultralytics и откатывал исправление.

Сегменты и ключевые точки патч не трогает: ultralytics их в albumentations не передаёт
(в его коде это отмечено как TODO), поэтому для задач segment и pose пространственные
трансформы по-прежнему рассинхронизируют полигоны и точки. Для detection, на который
рассчитан этот пайплайн, ограничение не проявляется.
...
Поэтому при передаче `augmentations.path` штатные аугментации, которые дублируют работу
пайплайна, автоматически отключаются (`hsv_h`, `hsv_s`, `hsv_v`, `bgr`, `degrees`,
`translate`, `shear`, `perspective`, `flipud`, `fliplr`). Многокадровые аугментации
(`mosaic`, `mixup`, `cutmix`, `copy_paste`) остаются как заданы: они склеивают несколько
изображений до пайплайна, и albumentations их не выражает.

`scale` отключается только при выключенном `mosaic`. При включённом он несёт служебную
функцию — `RandomPerspective` этим коэффициентом вписывает двойной холст мозаики обратно
в `imgsz`, и обнуление превратило бы мозаику в центральный кроп.

Из-за этого при включённой мозаике отключается и расписание `close_mosaic`: оно
пересобирает трансформы с `mosaic=0`, не трогая остальные гиперпараметры, так что
последние N эпох `scale` работал бы уже как обычный случайный зум поверх пайплайна.

Запрещённые комбинации приводят к ошибке до старта обучения:

| Комбинация | Почему |
|---|---|
| `+train_kwargs.fliplr=0.5` и т.п. вместе с пайплайном | изображение аугментировалось бы дважды |
| `+train_kwargs.close_mosaic=N` при включённой мозаике | в конце обучения режим аугментаций сменился бы на ходу |
| `+train_kwargs.augmentations=...` | поле конфига перезаписало бы это значение молча |

`+train_kwargs.cfg=hyp.yaml` не запрещается, но аугментации из этого файла перекрываются
явными аргументами — приложение об этом предупреждает.

Чтобы осознанно оставить оба стека: `keep_default_augmentations=true`.
