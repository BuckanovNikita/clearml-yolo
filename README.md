...
**Под WSL и в части контейнеров NVML не возвращает список процессов вообще**, как бы
занята карта ни была, — `max_compute_processes` там не видит соседа. Приложение
предупреждает об этом в логе, когда память занята, а процессов не названо; единственной
защитой в такой среде остаются `min_free_vram_gb` и `max_used_fraction`, и их стоит
ужесточить:
